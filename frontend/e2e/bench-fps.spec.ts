...
async function login(page: Page) {
  ...
  if (await passwordInput.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    // "YOUR NAME" field — second input on the form
    const usernameInput = page.locator('input[placeholder*="audit" i], input[name="username"]');
    if (await usernameInput.isVisible({ timeout: 1000 }).catch(() => false)) {
      await usernameInput.fill(USERNAME);
    }
    await page.locator('button:has-text("Continue"), button[type="submit"]').click();
    await page.waitForLoadState("networkidle");
